...
;(function () {
  ...
  function render(hits, q, total, err) {
    ...
    if (!hits.length) {
      if (
        typeof window.eqIsMarketReyonSeriesQuery === "function" &&
        window.eqIsMarketReyonSeriesQuery(q) &&
        typeof window.eqMarketReyonSeriesUrl === "function"
      ) {
        var mrUrl = window.eqMarketReyonSeriesUrl(q);
        grid.innerHTML =
          '<p class="eq-dept-plp-empty">«' +
          esc(q) +
          '» Çağlayan vitrin serisidir. Ürünleri <a href="' +
          esc(mrUrl) +
          '">Market reyonları</a> sayfasında görebilirsiniz.</p>';
      } else {
  ...
  function load() {
    ...
    if (
      ...
      var mr = window.eqMarketReyonSeriesUrl(q);
      if (mr) {
        location.replace(mr);
        return;
      }
