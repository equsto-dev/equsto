...
;(function () {
  ...
  /** Çağlayan market reyon vitrin serileri (nav: NİLÜFER, LOTUS, …) */
  var MARKET_REYON_SERIES = [
    "nilufer",
    "lotus",
    "nergis",
    "lale",
  ...
  function isMarketReyonSeriesQuery(q) {
    if (window.EqDeptTips && typeof window.EqDeptTips.resolveProsoSeriesTip === "function") {
      if (window.EqDeptTips.resolveProsoSeriesTip(q)) return true;
    }
    if (window.EqDeptTips && typeof window.EqDeptTips.resolveCaglayanSeriesTip === "function") {
      return !!window.EqDeptTips.resolveCaglayanSeriesTip(q);
    }
    var f = foldTr(trimQ(q));
    if (!f) return false;
    for (var i = 0; i < MARKET_REYON_SERIES.length; i++) {
      var s = MARKET_REYON_SERIES[i];
      if (f === s || f.indexOf(s) >= 0) return true;
    }
    return false;
  }

  function marketReyonSeriesTipId(q) {
    if (window.EqDeptTips && typeof window.EqDeptTips.resolveProsoSeriesTip === "function") {
      var proso = window.EqDeptTips.resolveProsoSeriesTip(q);
      if (proso) return proso;
    }
    if (window.EqDeptTips && typeof window.EqDeptTips.resolveCaglayanSeriesTip === "function") {
      var resolved = window.EqDeptTips.resolveCaglayanSeriesTip(q);
      if (resolved) return resolved;
    }
    if (window.EqDeptTips && typeof window.EqDeptTips.caglayanSeriesTipId === "function") {
      return window.EqDeptTips.caglayanSeriesTipId(q);
    }
    return "caglayan-" + foldTr(trimQ(q));
  }

  function marketReyonSeriesUrl(q) {
    q = trimQ(q);
    if (!q) return "";
    var tip = marketReyonSeriesTipId(q);
    if (typeof window.equstoUrl === "function") {
      try {
        return window.equstoUrl("marketReyon") + "?tip=" + encodeURIComponent(tip);
      } catch (_) {}
    }
    return "/shop/market-reyonlari?tip=" + encodeURIComponent(tip);
  }

  function aramaUrl(q) {
    q = trimQ(q);
    if (!q) return "";
    if (isMarketReyonSeriesQuery(q)) return marketReyonSeriesUrl(q);
  ...
  window.eqIsMarketReyonSeriesQuery = isMarketReyonSeriesQuery;
  window.eqMarketReyonSeriesUrl = marketReyonSeriesUrl;
