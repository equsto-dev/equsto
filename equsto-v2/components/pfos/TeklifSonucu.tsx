...
export default function TeklifSonucu({ sonuc }: Props) {
  const { kalemler, ozet, guvenSkoru, konseptLabel, m2, sehir, uyarilar } = sonuc;

  const gruplar = kalemler.reduce<Record<string, PFOSKalemi[]>>((acc, k) => {
    const key = k.kategoriKodu;
    if (!acc[key]) acc[key] = [];
    acc[key].push(k);
    return acc;
  }, {});
    ...
    <div style={s.wrap}>
      <div style={s.baslikBand}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={s.skorAlt}>
            {ozet.eslesmisZorunluSayisi}/{ozet.zorunluKalemSayisi} zorunlu eşleşti
          </div>
        </div>
      </div>

      {Object.entries(gruplar).map(([kat, items]) => (
        <div key={kat} style={s.grup}>
          <div style={s.grupBaslik}>
          ...
          <table style={s.tablo}>
            ...
            <tbody>
              {items.map((k) => (
                <tr key={k.poz} style={k.tip === "zorunlu" ? {} : s.opsRow}>
      ...
      <div style={s.toplamlBand}>
        ...
        {ozet.toplamFiyat != null && ozet.toplamFiyat > 0 && (
          <div style={s.toplam}>
