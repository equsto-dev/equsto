/**
 * Vercel tek giris — prebuild, admin-config, next build, .next repo kokune kopya.
 * Panel buildCommand: node scripts/vercel-ci.mjs
 */
...
function runNode(script, cwd = siteDir) {
...
function copyNextToRepoRoot() {
  ...
  if (!fs.existsSync(siteNext)) {
    console.error("[vercel-ci] HATA: .next yok:", siteNext);
    process.exit(1);
  }
  ...
  if (!fs.existsSync(manifest)) {
    console.error("[vercel-ci] HATA: routes-manifest-deterministic.json yok:", manifest);
    process.exit(1);
  }
  console.log("[vercel-ci] .next kopyalandi:", siteNext, "→", rootNext);
}
