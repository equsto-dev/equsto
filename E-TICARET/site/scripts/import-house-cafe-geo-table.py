#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""19-house-cafe.xlsx → public/data/geo/house-cafe-19-table.json"""
...
OUT_JSON = ROOT / "public" / "data" / "geo" / "house-cafe-19-table.json"
OUT_XLSX = ROOT / "public" / "data" / "geo" / "19-house-cafe.xlsx"
DEFAULT_XLSX = Path.home() / "Desktop" / "19-house-cafe.xlsx"
TITLE_PREFIX = "19-"
...
def convert(xlsx: Path) -> dict:
    ...
    for r in ws.iter_rows(min_row=1, max_col=15, values_only=True):
        ...
        if z and isinstance(z, str) and z.strip():
            zt = z.strip()
            if zt.lower() in ("p.no",) or zt.startswith(TITLE_PREFIX):
                continue
            if not cells[6]:
    ...
    return {
        "proformaNo": "19-house-cafe",
        "label": "All day casual cafe referans proforma (19- THE HOUSE CAFE)",
        "kaynakDosya": "19-house-cafe.xlsx",
