#!/usr/bin/env node
/**
 * Urban Bar Shopify katalog → Equsto vitrin
 *
...
const SRC_IMG = path.join(ROOT, "scripts/data/urbanbar/images");
const DEPT_DIR = path.join(ROOT, "public/data/dept");
const OUT_IMG = path.join(ROOT, "public/images/catalog/urbanbar");

const BRAND_DEFAULT = "Urban Bar";
const BRAND_ID = "urban-bar";
const KAYNAK = "urbanbar-web";
const KDV = Number(process.env.EQUSTO_KDV_ORAN || "20");
const SATIS_ORAN = Number(process.env.EQUSTO_URBANBAR_SATIS_ORAN || "1");
...
async function fetchTcmbGbpTry() {
  ...
  try {
    ...
    console.warn("[urbanbar-import] GBP fallback:", e?.message || e);
    return fallback;
  }
}

function pricingFromGbp(gbpInclVat, gbpTry, eurTry) {
  if (!gbpInclVat || gbpInclVat <= 0) return null;
  const listeGbp = Math.round(gbpInclVat * 100) / 100;
  const satisGbp = Math.round(listeGbp * SATIS_ORAN * 100) / 100;
...
async function toRows(p, gbpTry, eurTry) {
  ...
  return variants.map((variant) => {
    const px = pricingFromGbp(variant.priceGbp, gbpTry, eurTry);
...
async function main() {
  if (!fs.existsSync(SRC_JSON)) {
    console.error("Önce: node scripts/scrape-urbanbar-catalog.mjs");
    process.exit(1);
  }
  ...
  console.log(`[urbanbar-import] ${products.length} ürün, ${raw.collectionCount || "?"} koleksiyon`);
  ...
  for (const [dept, add] of Object.entries(byDept)) {
    ...
    let kept = [];
    if (fs.existsSync(file)) kept = JSON.parse(fs.readFileSync(file, "utf8")).filter((r) => !isUrbanBarRow(r));
  ...
  console.log(`[urbanbar-import] ${dryRun ? "DRY-RUN" : "OK"} ${rows.length} satır (${products.length} ürün)`);
  ...
  if (!dryRun) {
    ...
    console.log("\nYerel vitrin: http://localhost:3099/shop/marka/urban-bar");
  }
}
