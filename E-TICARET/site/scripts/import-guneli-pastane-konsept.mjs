/**
 * 2016-075 Güneli Fırın → pfos-referans + pfos-kategoriler.json
 * Kaynak: PFOS/veri/guneli-pastane-2016-075.xlsx
 * Kullanım: node scripts/import-guneli-pastane-konsept.mjs
 */
...
const KATEGORI_ID = "guneli-pastane";
const BANT_ID = "200-400";
const XLSX = "guneli-pastane-2016-075.xlsx";
...
/** Güneli teklif: col2=poz, col3=marka, col4=ürün, col5=ölçü, col6=adet */
function parseGuneliWs(ws) {
  const rows = [];
  let bolum = "";
  let bolumAd = "";
  ws.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    ...
    if (!poz && !ad) return;
    if (/^poz/i.test(poz) || /ürün|marka/i.test(ad)) return;
...
async function main() {
  ...
  const kalemler = parseGuneliWs(wb.worksheets[0]);
  ...
  const liste = {
    ...
    bantId: BANT_ID,
    label: "Güneli Fırın 200–400 m²",
    referansM2: REFERANS_M2,
    kaynakDosya: "2016-075 GÜNELİ FIRIN/2016-075.xlsx",
    not: "Pastane + yerel satış · Güneli Fırın markası",
