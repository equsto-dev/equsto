#!/usr/bin/env python3
"""
Equsto Product Canonical URL Audit
Checks canonical tags for all product URLs from sitemaps.
"""
...
CSV_PATH = OUTPUT_DIR / "product-url-http-audit.csv"
...
class CanonicalAuditor:
    def __init__(self):
        self.results: List[CanonicalResult] = []
        ...
        self.csv_path = OUTPUT_DIR / "product-canonical-audit.csv"
        self.csv_file = open(self.csv_path, 'w', newline='', encoding='utf-8')
        self.csv_writer = csv.writer(self.csv_file)
        self.csv_writer.writerow([
            'url', 'http_status', 'final_url', 'canonical', 'canonical_status',
            'canonical_matches_requested_url', 'canonical_matches_final_url',
            'canonical_is_absolute', 'canonical_host', 'canonical_protocol', 'issues'
        ])
        self.csv_file.flush()
    ...
    def normalize_url(self, url: str) -> str:
        ...
        parsed = urlparse(url)
        # Normalize: lower host, remove default ports, normalize path
    ...
    async def run_audit(self):
        ...
        print(f"Found {len(self.urls)} URLs to audit for canonical")
        ...
        self.csv_file.close()
        print(f"Canonical audit complete. Processed {len(self.results)} URLs.")
        return self.results
        ...
        with open(errors_path, 'w', encoding='utf-8') as f:
            for r in self.results:
                if r.canonical_status != "matching" or r.issues:
                    f.write(f"{r.url} | {r.canonical_status} | {r.canonical} | {r.issues}\n")
    ...
    def generate_summary(self) -> Dict:
        ...
        for r in self.results:
            ...
            if r.canonical and not r.canonical_is_absolute:
                invalid_canonical += 1
        ...
        # Find duplicate canonical groups
