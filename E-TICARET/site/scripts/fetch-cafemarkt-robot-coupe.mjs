#!/usr/bin/env node
/**
 * Cafemarkt Robot Coupe marka araması → JSON (ld+json ItemList)
...
const OUT = path.join(ROOT, "scripts/data/cafemarkt-robot-coupe.json");
const UA =
...
function parseItemList(html) {
  ...
  return (data.itemListElement || []).map((li) => {
    const p = li.item || {};
    const brand = p.brand?.name || "Robot Coupe";
    return {
...
async function main() {
  ...
  console.log(`[cafemarkt-rc] toplam ~${first.total} urun, ${lastPg} sayfa`);

  for (let pg = 1; pg <= lastPg; pg++) {
    const page = pg === 1 ? first : await fetchPage(pg);
    for (const row of page.items) {
      if (!/robot coupe/i.test(`${row.brand} ${row.name}`)) continue;
      const key = row.cafemarkt_id || row.code || row.url;
      map.set(key, row);
    }
    console.log(`[cafemarkt-rc] sayfa ${pg}: +${page.items.length} → birlesik ${map.size}`);
  ...
  console.log(`[cafemarkt-rc] yazildi: ${OUT} (${rows.length} urun)`);
}
