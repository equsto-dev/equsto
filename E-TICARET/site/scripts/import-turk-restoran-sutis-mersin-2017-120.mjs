/**
 * 2017-120 Sütiş Mersin → turk-restoran (kahvaltı + öğle/akşam) referans
 * Kaynak: PFOS/veri/turk-restoran-sutis-mersin-2017-120.xlsx
 * Kullanım: npm run pfos:sutis-mersin-turk-restoran:import
 *
...
const BANT_ID = "sutis-mersin";
const XLS = "turk-restoran-sutis-mersin-2017-120.xlsx";
const REFERANS_M2 = 450;

const POZ_RE = /^[A-Z]\d+[A-Z]?$/i;
...
function parseTeklifWs(ws) {
  ...
  ws.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    ...
    // Bölüm satırları: "S- SERVİS TEŞHİR" gibi
...
async function upsertManifest(meta) {
  ...
  const mersinBant = {
    id: BANT_ID,
    label: "Sütiş Mersin (2017-120)",
    referansM2: REFERANS_M2,
    meta,
  };
  const bi = bantlar.findIndex((b) => b.id === BANT_ID);
  if (bi >= 0) bantlar[bi] = mersinBant;
  else bantlar.push(mersinBant);

  const kayit = {
    id: KATEGORI_ID,
    label: prev?.label ?? "Türk Restoranı",
    ustKategori: prev?.ustKategori ?? "Restoran",
    bantlar,
  };
...
async function main() {
  ...
  const liste = {
    ...
    bantId: BANT_ID,
    label: "Türk restoranı (Sütiş Mersin) — kahvaltı + öğle/akşam",
    referansM2: REFERANS_M2,
    kaynakDosya: "2017-120 SÜTİŞ MERSİN/2017-120.xlsx",
    not: "Sütiş Mersin · kahvaltı - öğle ve akşam yemekleri · Türk mutfağı referansı · 2017-120",
    konseptSinif: "turk-restoran-sutis-mersin",
    yukleme,
