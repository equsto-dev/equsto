#!/usr/bin/env node
/**
 * cafemarkt-ozti.json → images-ozti klasörüne görsel kopyala.
 *   node scripts/copy-cafemarkt-ozti-images.mjs
 */
...
const JSON_PATH = path.join(ROOT, "scripts/data/cafemarkt-ozti.json");
const SRC_DIR =
...
for (const f of files) {
  ...
  if (fs.existsSync(src)) {
    ...
    miss++;
    if (missList.length < 15) missList.push(f);
  }
}

console.log(`Öztiryakiler ürün: ${products.length}`);
