/**
 * 2017-106 Sütiş Koşuyolu → turk-restoran 250–500 m² referans
 * Kaynak: PFOS/veri/turk-restoran-sutis-kosuyolu-2017-106.xlsx
 * Kullanım: npm run pfos:sutis-kosuyolu-turk-restoran:import
 *
...
const BANT_ID = "250-500";
const XLS = "turk-restoran-sutis-kosuyolu-2017-106.xlsx";
...
async function upsertManifest(meta) {
  ...
  const kosuyoluBant = {
    id: BANT_ID,
    label: "250–500 m² (Sütiş Koşuyolu)",
    referansM2: REFERANS_M2,
    meta,
  };
  const bi = bantlar.findIndex((b) => b.id === BANT_ID);
  if (bi >= 0) bantlar[bi] = kosuyoluBant;
  else bantlar.push(kosuyoluBant);
...
async function main() {
  ...
  const liste = {
    ...
    bantId: BANT_ID,
    label: "Türk restoranı 250–500 m² (Sütiş Koşuyolu)",
    referansM2: REFERANS_M2,
    kaynakDosya: "2017-106 SÜTİŞ KOŞUYOLU/2017-106.xlsx",
    not: "Sütiş Koşuyolu · servis teşhir + servis mutfağı · Türk mutfağı referansı · 2017-106",
    konseptSinif: "turk-restoran-sutis-kosuyolu",
    yukleme,
