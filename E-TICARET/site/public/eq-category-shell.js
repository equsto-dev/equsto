...
(function (global) {
  ...
  function mount(opts) {
    ...
    var tiles = Array.isArray(opts.tiles) ? opts.tiles : [];

    var state = { active: tiles.length ? String(tiles[0].slug || "") : "" };

    function filtered(list) {
      var out = list.filter(function (x) { return x && predicate(x); });
      if (state.active) out = out.filter(function (x) { return String(x.category || "") === state.active; });
      return out;
    }
    ...
    function render(all) {
      ...
      var hero =
        '<section class="eq-cat-hero"><h1>' + esc(catLabel) + "</h1>" +
        (catDesc ? "<p>" + esc(catDesc) + "</p>" : "") +
        "</section>";
      ...
      var grid =
        list.length
          ? '<div class="eq-cat-grid">' + list.map(renderCard).join("") + "</div>"
          : '<div class="eq-cat-empty">Bu markada ürün bulunamadı.</div>';

      root.innerHTML = hero + tilesHtml + grid;
    ...
    loader
      .then(function (all) {
        ...
        // If no initial active, choose most frequent
        if (!state.active && tiles.length) state.active = String(tiles[0].slug || "");
        render(all || []);
      })
      .catch(function () {
