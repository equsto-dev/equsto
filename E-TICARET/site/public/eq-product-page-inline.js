...
window.searchFilter = window.searchFilter || function () {};
    ...
    function renderInoksanFeaturesCol(x) {
    ...
    function oztiFuelProfile(x) {
      var lines = Array.isArray(x.teknik_ozellikler) ? x.teknik_ozellikler : [];
      var blob = [x.name, x.sku, x.model].filter(Boolean).join(" ");
      lines.forEach(function (ln) {
        var t = String(ln || "");
        if (/ürün tipi|ürün adı/i.test(t)) blob += " " + t;
      });
      var low = blob.toLocaleLowerCase("tr");
      var hasElkFields = lines.some(function (ln) {
        return /^elektrik\s/i.test(String(ln || "").trim());
      });
      var hasGazFields = lines.some(function (ln) {
        var k = String(ln || "")
          .split(":")[0]
          .trim()
          .toLowerCase();
        return /^gaz\s/.test(k) || /gaz gücü|gaz girişi|gaz basınç|doğalgaz|dogalgaz/.test(k);
      });
      var tipLine = "";
      lines.forEach(function (ln) {
        if (/^ürün tipi:/i.test(String(ln || "").trim())) tipLine = String(ln);
      });
      var tip = tipLine
        ? tipLine
            .split(":")
      ...
      var electric =
        hasElkFields ||
        /elektrikli|elektrik\s|f[ıi]r[ıi]n\s*elektrik|[-_.]50e\b/i.test(low);
      var gas =
        hasGazFields ||
        /gazl[ıi]|\bgaz\b.*ocak|brülör|brulor|açık ateş|acik ates|doğalgaz|dogalgaz|\blpg\b/i.test(
          low
        );
      ...
      if (tip === "elektrikli") {
        electric = true;
        gas = false;
      } else if (tip === "gazlı" || tip === "gazli") {
        gas = true;
        if (
          !hasElkFields &&
          !/elektrikli|f[ıi]r[ıi]n\s*elektrik|[-_.]50e\b/i.test(low)
        ) {
          electric = false;
        }
      }
    ...
    function oztiConnectionSpecs(x) {
      var profile = oztiFuelProfile(x);
      var lines = Array.isArray(x.teknik_ozellikler) ? x.teknik_ozellikler : [];
      var elk = [];
      var gaz = [];
      var su = [];
      ...
      lines.forEach(function (ln) {
        var t = decodeHtmlEntitiesPdp(String(ln || "").trim());
        if (!t || t.indexOf(":") < 0 || t.length > 200) return;
        ...
        if (/^g[uü][çc]$|^guc$/.test(key)) {
          ...
          else if (profile.gas) push(gaz, gucLine);
          else push(diger, t);
        }
      });
    ...
    function renderOztiFeaturesCol(x) {
      ...
      var conn = oztiConnectionSpecs(x);
      var acc = "";
      if (temel.length) {
        acc += renderEpdpFlatAccSection(
          __pdpT("pdp.basic_info", "Temel bilgiler"),
      ...
      if (!acc) {
        acc =
          ...
          esc(
            __pdpT(
              "pdp.specs_request_quote",
              "Detaylı teknik özellikler için teklif sürecinden talep edebilirsiniz."
            )
          ) +
          "</p>";
      }
    ...
    function isPimakCatalogRow(x) {
      if (!x) return false;
      if (x.pimak_web_description || x.pimak_slug || x.pimak_gorsel) return true;
      if (/^pimak__/i.test(String(x.id || ""))) return true;
      var kaynak = String(x.kaynak || x.kaynak_fiyat_listesi || "").toLowerCase();
      if (kaynak === "pimak" || kaynak.indexOf("pimak") >= 0) return true;
      return /^pimak$/i.test(String(x.brand || "").trim());
    }

    function parsePimakGucKwValue(raw) {
      var s = decodeHtmlEntitiesPdp(String(raw || "").trim());
      if (!s || /iletisim|contact/i.test(s)) return null;
      var kwPart = s.split("/")[0].trim();
      var nums = [];
      var m;
      var re = /(\d+(?:[.,]\d+)?)/g;
      while ((m = re.exec(kwPart))) {
    ...
    function pimakKwSuffix(val) {
      var kw = typeof val === "number" ? val : parsePimakGucKwValue(val);
      if (kw == null) return oztiKwSuffix(val);
      var s = String(kw);
      return (s.indexOf(".") >= 0 ? s.replace(".", ",") : s) + " kW";
    }

    function pimakTeknikMarketingLines(x) {
      return (Array.isArray(x.teknik_ozellikler) ? x.teknik_ozellikler : [])
        .map(function (ln) {
          return decodeHtmlEntitiesPdp(String(ln || "").trim());
        })
        .filter(function (t) {
          return t.length > 3 && t.indexOf(":") < 0;
        });
    }

    function isPimakGucSpecKey(key) {
      return /^g[uü][çc](?:\s*\([^)]*\))?$|^guc$/i.test(String(key || "").trim());
    }

    function pimakDescriptionBullets(x) {
      var raw = decodeHtmlEntitiesPdp(
        String(x.pimak_web_description || x.aciklama || x.description || "").trim()
      );
      if (raw) {
        var parts;
        if (/\n\s*[•·\-–—*]/.test(raw)) {
        ...
        var fromWeb = parts
          ...
          .filter(function (l) {
            return l.length > 3 && !/^tasar[iı]m:?$/i.test(l);
          });
        if (fromWeb.length) return fromWeb;
      }
      return pimakTeknikMarketingLines(x);
    }

    function renderPimakDescriptionCol(x) {
      var bullets = pimakDescriptionBullets(x);
      if (!bullets.length) return "";
      return renderEpdpDocsPanel(
        __pdpT("pdp.product_description", "Ürün açıklaması"),
        '<div class="eq-ozti-desc-panel">' +
    ...
    function pimakFuelProfile(x) {
      var lines = Array.isArray(x.teknik_ozellikler) ? x.teknik_ozellikler : [];
      var blob = [x.name, x.sku, x.model].filter(Boolean).join(" ").toLocaleLowerCase("tr-TR");
      var gas = false;
      var electric = false;
      lines.forEach(function (ln) {
        var t = decodeHtmlEntitiesPdp(String(ln || "").trim());
        var key = t.split(":")[0].trim().toLocaleLowerCase("tr-TR");
        if (/^enerji tipi:/.test(key)) {
          var tip = t.split(":").slice(1).join(":").trim().toLocaleLowerCase("tr-TR");
          if (/gaz|lpg|doğalgaz|dogalgaz/.test(tip)) gas = true;
          if (/elektr/.test(tip)) electric = true;
        }
        if (/^gaz bas/i.test(key)) gas = true;
        if (/^voltaj/.test(key)) electric = true;
        if (isPimakGucSpecKey(key)) {
          if (x.gaz_guc != null && x.gaz_guc > 0) gas = true;
          else if (x.el_guc != null && x.el_guc > 0) electric = true;
          else if (/gaz|mbar|lpg|brülör|brulor/.test(blob)) gas = true;
          else electric = true;
        }
      });
      if (x.gaz_guc != null && x.gaz_guc > 0) gas = true;
      if (x.el_guc != null && x.el_guc > 0) electric = true;
      if (!gas && !electric) {
        if (/elektr|380\s*v|230\s*v|220\s*v|fritoz|firin|fırın|mikser|buz|ocak|izgar|y[ıi]kama|bula[sş][ıi]k/.test(blob)) {
          electric = true;
        }
      }
      return { gas: gas, electric: electric };
    ...
    function pimakConnectionSpecs(x) {
      var profile = pimakFuelProfile(x);
      var lines = Array.isArray(x.teknik_ozellikler) ? x.teknik_ozellikler : [];
      var elk = [];
      var gaz = [];
      var diger = [];
      ...
      lines.forEach(function (ln) {
        ...
        if (isPimakGucSpecKey(key)) {
          push(profile.gas && !profile.electric ? gaz : elk, "Güç: " + pimakKwSuffix(val));
          return;
        }
        if (/^voltaj/.test(key)) {
          push(elk, "Voltaj: " + val);
        ...
        if (/^gaz bas/.test(key)) {
          push(gaz, "Gaz basıncı: " + val);
          return;
        }
        if (/^motor g[uü][çc]/.test(key)) {
          var motorKw = parsePimakGucKwValue(val);
          push(diger, motorKw != null ? "Motor gücü: " + pimakKwSuffix(motorKw) : "Motor gücü: " + val);
          return;
        }
        if (/^enerji tipi/.test(key)) return;
      });

      if (profile.electric && !elk.some(function (l) { return /^Güç:/.test(l); }) && x.el_guc > 0) {
        push(elk, "Güç: " + pimakKwSuffix(x.el_guc));
      }
      if (profile.gas && !gaz.some(function (l) { return /^Güç:/.test(l); }) && x.gaz_guc > 0) {
        push(gaz, "Güç: " + pimakKwSuffix(x.gaz_guc));
      }

      return { profile: profile, elk: elk, gaz: gaz, diger: diger };
    }

    function renderPimakFeaturesCol(x) {
      var ref = deptLink(x.category, x.dept);
      var temel = [];
      var visBrand = pdpVisibleBrand(x.brand);
      if (visBrand) temel.push(__pdpT("pdp.brand_prefix", "Marka:") + " " + visBrand);
      ...
      (Array.isArray(x.teknik_ozellikler) ? x.teknik_ozellikler : []).forEach(function (ln) {
        ...
        var key = t.split(":")[0].trim().toLocaleLowerCase("tr-TR");
        if (
          isPimakGucSpecKey(key) ||
          /^voltaj|^gaz bas|^motor g|^enerji tipi|^ebat|^a[gğ]ırlık|^kapasite|^en \(|^boy \(|^ürün kodu/.test(
            key
          )
        ) {
          return;
      ...
      var conn = pimakConnectionSpecs(x);
      var acc = "";
      if (temel.length) {
        acc += renderEpdpFlatAccSection(
          __pdpT("pdp.basic_info", "Temel bilgiler"),
    ...
    function isAtalayCatalogRow(x) {
      if (!x) return false;
      if (x.atalay_web_description || x.atalay_description_at || x.atalay_web_url) return true;
      if (/atalay/i.test(String(x.brand || ""))) return true;
      var id = String(x.id || "").toLowerCase();
      if (id.indexOf("atalay-endustriyel-mutfak-ekipmanlari__") === 0) return true;
      var kaynak = String(x.kaynak || x.kaynak_fiyat_listesi || "").toLowerCase();
      return kaynak.indexOf("atalay") >= 0;
    }

    function atalayDescriptionBullets(x) {
      var raw = decodeHtmlEntitiesPdp(
        String(x.atalay_web_description || x.description || "").trim()
      );
      if (!raw) return [];
      var parts;
      if (/\n\s*[•·\-–—*]/.test(raw)) {
    ...
    function renderAtalayDescriptionCol(x) {
      var bullets = atalayDescriptionBullets(x);
      if (!bullets.length) return "";
      return renderEpdpDocsPanel(
        __pdpT("pdp.product_description", "Ürün açıklaması"),
        '<div class="eq-ozti-desc-panel">' +
    ...
    function atalayFuelProfile(x) {
      var lines = Array.isArray(x.teknik_ozellikler) ? x.teknik_ozellikler : [];
      var blob = [x.name, x.sku, x.model, x.atalay_web_slug, x.atalay_web_title]
        .filter(Boolean)
        .join(" ");
      lines.forEach(function (ln) {
        var t = String(ln || "");
        if (/ürün tipi|ürün adı|enerji/i.test(t)) blob += " " + t;
      });
      var low = blob.toLocaleLowerCase("tr");
      var hasElkFields = lines.some(function (ln) {
        var k = String(ln || "")
          ...
          .trim()
          .toLocaleLowerCase("tr");
        return /^elektrik\s/.test(k) || /gerilim|frekans|amper|fiş/.test(k);
      });
      var hasGazFields = lines.some(function (ln) {
        var k = String(ln || "")
          .split(":")[0]
        ...
        return (
          /^gaz\s/.test(k) ||
          /gaz sarfiyat|gaz gücü|gaz girişi|gaz basınç|doğalgaz|dogalgaz|lpg|\bng\b/.test(k)
        );
      });

      var electric =
        hasElkFields ||
        /elektrikli|elektrik\s|f[ıi]r[ıi]n\s*elektrik|-e-|-elektrikli/i.test(low);
      var gas =
        hasGazFields ||
        /gazl[ıi]|\bgaz\b.*ocak|brülör|brulor|açık ateş|acik ates|doğalgaz|dogalgaz|\blpg\b|-g-|-gazli/i.test(
          low
        );

      if (/elektrikli/.test(low) && !/gazl[ıi]|dogalgaz|lpg|gaz sarfiyat/.test(low)) {
        electric = true;
        gas = false;
      } else if (/gazl[ıi]|dogalgaz|lpg/.test(low) && !hasElkFields && !/elektrikli|gerilim/.test(low)) {
        gas = true;
        electric = false;
      }
    ...
    function atalayConnectionSpecs(x) {
      var profile = atalayFuelProfile(x);
      var lines = Array.isArray(x.teknik_ozellikler) ? x.teknik_ozellikler : [];
      var elk = [];
      var gaz = [];
      var diger = [];
      ...
      lines.forEach(function (ln) {
        ...
        if (/gerilim|voltaj|frekans|amper|fiş/.test(key)) {
          if (/gerilim|volt/.test(key)) push(elk, "Voltaj: " + val);
          else if (/frekans/.test(key)) push(elk, "Frekans: " + val);
          else push(elk, t);
          return;
        }
    ...
    function renderAtalayFeaturesCol(x) {
      var ref = deptLink(x.category, x.dept);
      var temel = [];
      var visBrand = pdpVisibleBrand(x.brand);
      if (visBrand) temel.push(__pdpT("pdp.brand_prefix", "Marka:") + " " + visBrand);
      ...
      var conn = atalayConnectionSpecs(x);
      var acc = "";
      if (temel.length) {
        acc += renderEpdpFlatAccSection(
          __pdpT("pdp.basic_info", "Temel bilgiler"),
    ...
    function pdpLeadParagraph(x) {
      ...
      if (isPimakCatalogRow(x) && pimakDescriptionBullets(x).length) return "";
      if (isAtalayCatalogRow(x) && atalayDescriptionBullets(x).length) return "";
      var desc =
        window.eqLang === "en" && x.descriptionEn && String(x.descriptionEn).trim()
          ? String(x.descriptionEn).trim()
    ...
    function applyEpdpMobileBuybarInlineStyles(bar) {
      if (!bar) return;
      ...
      bar.style.setProperty("box-shadow", "0 -4px 16px rgba(0, 0, 0, 0.1)", "important");
      bar.style.setProperty("border-top", "1px solid #e5e7eb", "important");
      ...
      var inner = bar.querySelector(".eq-pdp-mobile-buybar__inner");
      if (inner) {
    ...
    function scheduleEpdpMobileBuybarRetry(cartU) {
      if (!cartU) return;
      var delays = [80, 250, 700, 1800];
      for (var i = 0; i < delays.length; i++) {
        (function (delay) {
          setTimeout(function () {
            if (!isEpdpMobileBuybarViewport()) return;
            if (document.getElementById("eq-pdp-mobile-buybar")) return;
            var heroBox =
              document.querySelector(".eq-epdp-hero .eq-cmf-buybox") ||
              document.querySelector(".eq-cmf-buybox");
            if (!heroBox) return;
            mountEpdpMobileBuybar(heroBox, window.__eqEpdpLastCartU || cartU);
          }, delay);
        })(delays[i]);
      }
    }
    ...
    function mountEpdpMobileBuybar(box, cartU) {
      if (!box || !cartU || !isEpdpMobileBuybarViewport()) {
        clearEpdpMobileBuybar();
        return;
      }
      var existing = document.getElementById("eq-pdp-mobile-buybar");
      ...
      bar.innerHTML =
        ...
        "</div></div></div>";
      applyEpdpMobileBuybarInlineStyles(bar);
      document.body.appendChild(bar);
      document.body.classList.add("eq-pdp-mobile-buybar-on");
      hideEpdpTabbarForBuybar();
        ...
        try {
          if (window.visualViewport) {
            window.visualViewport.addEventListener("resize", function () {
              ...
              if (heroBox && lastU && isEpdpMobileBuybarViewport()) {
                mountEpdpMobileBuybar(heroBox, lastU);
              } else if (!isEpdpMobileBuybarViewport()) {
                clearEpdpMobileBuybar();
              }
            });
          }
    ...
    function bindEpdpBuybox(cartU) {
      ...
      scheduleEpdpMobileBuybarRetry(cartU);
    ...
    function renderEpdpFeaturesCol(x) {
      ...
      if (isInoksanCatalogRow(x)) return renderInoksanFeaturesCol(x);
      if (isPimakCatalogRow(x)) return renderPimakFeaturesCol(x);
      if (isOztiCatalogRow(x)) return renderOztiFeaturesCol(x);
      if (isAtalayCatalogRow(x)) return renderAtalayFeaturesCol(x);
      var ref = deptLink(x.category, x.dept);
      var temel = [];
      var visBrand = pdpVisibleBrand(x.brand);
      if (visBrand) temel.push(__pdpT("pdp.brand_prefix", "Marka:") + " " + visBrand);
    ...
    function renderEpdpDocsCol(x) {
      ...
      if (isPimakCatalogRow(x)) {
        var pimakDesc = renderPimakDescriptionCol(x);
        if (pimakDesc) return pimakDesc;
      }
      if (isAtalayCatalogRow(x)) {
        var atalayDesc = renderAtalayDescriptionCol(x);
        if (atalayDesc) return atalayDesc;
      }
      if (isCaglayanRefrigeration(x)) {
    ...
    function renderEpdpProduct(x, all) {
      ...
      if (root) {
        root.className =
          ...
          (isPimakCatalogRow(x) ? " eq-epdp--pimak eq-epdp--ozti" : "") +
          (isAtalayCatalogRow(x) ? " eq-epdp--atalay" : "");
      }
