...
export function isBuzdolabiReferansIsim(isim: string | null | undefined): boolean {
  ...
  return (
    /buzdolab|donduruc|derin\s*dondur|deep\s*freeze|freezer|sogutuc|soğutuc|sishe\s*sogut|şişe\s*soğut|icecek\s*sogut|içecek\s*soğut|bar\s*sogut|saladette|pizza\s*prep|sogutmali\s*tezgah|soğutmali\s*tezgah|hazirlik\s*buzdolab|hazırlık\s*buzdolab|bizdlab|bizlab|buzdlab/.test(
      n,
    )
  );
}
