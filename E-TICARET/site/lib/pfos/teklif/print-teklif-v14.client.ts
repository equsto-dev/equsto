...
function buildPrintHtml(model: TeklifModelV14): string {
  ...
  for (const block of blocks) {
    ...
    for (const row of block.satirlar) {
      ...
      const imgUrl = row.fotoUrl ? absImageUrl(row.fotoUrl) : "";
...
<body>
  <div class="head">
    <div>
      <img class="logo" src="${typeof window !== "undefined" ? absImageUrl("/images/equsto-logo.png") : "/images/equsto-logo.png"}" alt="EQUSTO">
...
/** Görselli teklifi yazdır / PDF olarak kaydet */
export function printTeklifV14(model: TeklifModelV14) {
  const html = buildPrintHtml(model);
  const w = window.open("", "_blank");
  if (!w) {
    alert(
      "Pop-up engellendi. Tarayıcıda pop-up izni verin veya Excel indirmeyi kullanın.",
    );
    return;
  }
  w.document.open();
